...
namespace MusicPlaylistExtractor.Services
{
    public class AmazonPlaylistScraper
    {
        public static async Task<MusicPlaylist> ScrapePlaylistAsync(string url)
        {
            ...
            var response = await client.SendAsync(RequestTemplate(playlistId));
            response.EnsureSuccessStatusCode();
            var jsonResponse = await response.Content.ReadAsStringAsync();

            return ExtractPlaylistFromJsonResponse(jsonResponse);
        }

        private static MusicPlaylist ExtractPlaylistFromJsonResponse(string jsonResponse)
        {
        ...
        private static HttpRequestMessage RequestTemplate(string playlistId)
        {
